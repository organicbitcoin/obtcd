...
“Organic Bitcoin”不是“有机食品比特币”，而是指**一个有新陈代谢的比特币系统**。

对比来看：

- Bitcoin 更像“无机系统”：UTXO 一旦创建就可以永久静止；状态会积累；安全预算长期受减半倒计时约束。
- OBTC 更像“有机系统”：资产会经历创建、使用、沉睡、到期、回收、再生；状态可以通过 REAP 自我清理；矿工收入多了来自沉睡供给回流的内生来源。

因此，“有机”强调的不是营销词，而是**生命周期、循环利用和系统自维持能力**。

## 1.2 创新位置：不在单项技术，在机制组合

OBTC 使用的很多底层原语并不是全新发明，例如：
...
它的创新点不主要在“新密码学”，而在于把这些机制组合成一个**可执行的经济系统**，直接回答一个长期被回避的问题：

> 如果链上永久状态不是免费的，谁来为它持续付费？
...
- `W = WindowBlocks`
- Mainnet `W = 362,880` 块（`9!`，约 `6.91` 年；对外沟通时常近似称“约 7 年”）
...
## 11.5 面向 AI agent 的钱包方向

OBTC 在钱包层还有一个自然延伸方向：**让生命周期资产管理更适合机器参与者，而不只适合人类手工操作员**。

原因很直接：

- agent 不能自行完成 KYC，也不能走传统银行开户流程；
- 但它们会越来越多地承担支付算力、购买数据、执行预算、管理金库等任务；
- 而 OBTC 的核心动作恰好是持续观察、按规则续期、批量调度、留痕审计，这些都更接近机器擅长的工作方式。

因此，钱包与接口层的优先方向应包括：

- capability-based 授权，而不是“解锁后拥有全部权限”；
- `preview -> approve -> sign -> publish` 的受控执行链路；
- watch-only planner 与 isolated signer 分层；
- 面向事件流的到期/失败/策略告警；
- 默认可审计的 operation metadata 与 policy metadata。

这一方向属于**钱包与产品层路线**，不是新的共识强制规则；接口形态仍可继续演化。
...
设主网 `W=362,880` 块，按 `52,560` 块/年换算，则：

\[
T = W / 52,560 \approx 6.91 \text{ 年}
\]

对单次 REAP 后的保留比例 `ρ=0.7`，有：

\[
p = -\ln(\rho)/T \approx 5.2\%/年
\]
...
## 12.4 历史继承带来的即时到期 backlog

OBTC 不是从空白创世块启动，而是继承 Bitcoin 的历史 UTXO 集。
...
且在激活时仍未花费的历史 UTXO，会在 Expiry 规则生效时直接进入 expired backlog。
...
- `A = 1,050,000`
- `W = 362,880`
- 因此阈值为 `h_create <= 687,120`

这意味着系统**不需要再等待一个完整的 6.91 年窗口**，才出现第一批可处理的历史沉睡输出。进入激活高度后，链上会立刻拥有一个预先存在的候选池。

需要注意的是：

- backlog 是“立即存在”的；
- 但释放是“逐块进行”的，因为单笔 REAP 仍受 `ReapMaxInputs` 和 weight budget 约束；
- 因而它更像一个持续多块消化的历史库存，而不是单块一次性清空。

## 12.5 经济飞轮：为什么它可能自启动

OBTC 的潜在飞轮不是单点叙事，而是多环节联动：

1. 历史沉睡输出形成即时 backlog  
2. backlog 经过 REAP 逐步转化为税收流  
3. 税收流增强矿工收入与安全预算韧性  
4. 更稳定的安全预期提升矿工、钱包、交易平台的接入意愿  
5. 更多接入又提升链上续期、转移和资产管理活动

如果再叠加 AI agent 的采用，这个飞轮还会多出一层真实需求：

- agent 需要无需许可、机器可调用、可授权、可审计的结算资产；
- OBTC 的 lifecycle model 恰好把“持续监控 + 策略执行 + 风险留痕”做成了一等公民。
...
- 主网时间窗口：`T = W / 52,560`，其中 Mainnet `W = 362,880`
- 年化衰减：`p = -lnρ / T`
- 安全流入：`B = L × p`
- 等预算变换：`ρ' = exp(-pT')`
...
因为它把资产看成有生命周期的系统，而不是永久静止的无机状态堆积。UTXO 会经历创建、使用、沉睡、到期、回收和再生；状态与安全预算都带有“代谢”含义。“Organic”强调的是活系统，不是有机食品。

### Q12：为什么白皮书会提 AI agent？

因为 OBTC 的钱包层天然需要持续监控到期窗口、批量续期、执行策略、保留审计记录，这些都很适合机器参与者。这里说的是**钱包与产品方向**，不是把 AI 写进共识规则。

---

## 21. 结语

OBTC 的核心不在“概念新奇”，而在“规则可执行”。它的独特点也不主要来自新密码学原语，而来自把已知机制组合成一个可运行的经济系统。
