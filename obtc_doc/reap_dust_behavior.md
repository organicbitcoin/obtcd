...
## 2) 778/779 cliff（边界突变）

在默认参数下：

- `value=778`：`tax=233`，`refund=545`（<546）→ 折叠后 `refund=0`、`tax=778`
- `value=779`：`tax=233`，`refund=546`（==546）→ 不折叠，`refund=546`、`tax=233`

结论：**只差 1 sat，结果从“无返还”跳到“返还 546”**。
...
- 每笔：`tax=210`，`refund=490`（<546）→ 两笔都折叠
- 最终：`TaxTotal=1400`，`RefundTotal=0`

如果误按“先聚合后判定”（**当前实现不是这样**）：

- 总额 1400：`tax=420`，`refund=980`（>=546）
- 会得到完全不同的结果。
...
- `value=545`：`refund=545`（<546）→ 折叠为 tax，最终 `refund=0`、`tax=545`
- `value=546`：`refund=546`（==546）→ 不折叠，`refund=546`
...
2. 对 `refund` 接近阈值（如 540~560 sat）做高亮提示，说明存在 cliff。
3. 在 UI/文档明确区分：
   - 名义税率（TaxNum/TaxDen）
   - dust 折叠导致的额外有效损失
