# OBTC Public Testnet User Test

This page is for people helping try the OBTC public testnet.
...
5. Request a small amount of testnet coin through a GitHub issue.
...
```text
Please help me try the OBTC public testnet on this machine.
...
- Start obtcd, wait until it has peers and is synced, then create/start
  btcwallet.
- Generate one testnet receive address and stop so I can request testnet coins.
...
1. After generating a receive address, so you can request testnet coins.
...
is not the preferred path for this test round.
...
There is no public faucet during this test stage. Testnet coins are sent
manually after a maintainer reviews the request.
...
Open a coin request issue:

```text
https://github.com/organicbitcoin/obtcd/issues/new?template=public_testnet_coin_request.md
```
...
Please send a short note. A few sentences are enough. You can reply in the same
coin request issue, or open a feedback issue:

```text
https://github.com/organicbitcoin/obtcd/issues/new?template=public_testnet_feedback.md
```
...
For a reproducible bug, use:

```text
https://github.com/organicbitcoin/obtcd/issues/new?template=public_testnet_bug.md
```
