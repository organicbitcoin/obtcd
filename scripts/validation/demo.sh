...
# OBTC Expiry Validation Demo Script
# This script demonstrates how to validate expiry index RPC behavior.
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE} OBTC Expiry Validation Demo${NC}"
echo -e "${BLUE}========================================${NC}"
echo
...
echo "2. Start OBTCD in OBTC regtest mode with expiry index enabled"
...
if pgrep -f "obtcd.*regtest.*expiryindex" > /dev/null; then
    ...
    echo -e "${BLUE}ℹ️  Starting OBTCD in OBTC regtest mode with expiry index...${NC}"
...
if /tmp/obtc_validator -rpcuser=test -rpcpass=test -network=obtcregtest -verbose; then
    ...
    echo "✅ OBTC expiry RPC behavior is working as expected"
else
    echo
...
echo "• Try the validation on testnet: ./quick_validate.sh obtctestnet --rpcuser=your_user --rpcpass=your_pass"
...
echo
echo -e "${GREEN}OBTC expiry validation completed successfully! ✨${NC}"
