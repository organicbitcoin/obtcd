...
Options:
  --quick        Run the default local profile (build + coverage + race + OBTC smoke + rpctest + quality).
...
main() {
    ...
    if [[ "${run_main_workflow}" -eq 1 ]]; then
        if [[ "${validation_profile}" == "quick" ]]; then
            print_warn "Quick profile enabled: skipping build-matrix."
